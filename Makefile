...
SUBS:=shell tools core lootto
cx:
	$(foreach d,$(SUBS),$(MAKE) -C opt/$d cc;)
rx:
	$(foreach d,$(SUBS),$(MAKE) -C opt/$d ru;)
sx:
	$(foreach d,$(SUBS),$(MAKE) -C opt/$d rstat;)
#	$(MAKE) -C opt/shell cc
#	$(MAKE) -C opt/tools cc
#	$(MAKE) -C opt/core cc
#	$(MAKE) -C opt/lootto cc
#	$(true)
...
#uc: $(BLD)/$H.cs
#	$(eval args!=jq -R '{c:.}' $<)
#	$(TOC) -c $(ETC)/$H.conf callx -m upgrade '$(args)'

#uc0: $(UOBJ)/stand/$I.cs
#uc: $(BLD)/$I.cs $(ETC)/$I.conf
#	$(eval args!=jq -R '{c:.}' $<)
...
#m=export_env
#d:
#	$(SOLD) -I $(INC_PATH) -I $(LIB_PATH) -I $(SYS_LIB_PATH) $f.sol
...
#	$(LINKER) test $f --decode-c6 --gas-limit 100000000 --abi-json $f.abi.json --abi-method $m --abi-params {}

...
library err {
    ...
    function strerror(uint8 ec) internal returns (string) {
        ...
        if (ec == EDEADLK) return "Resource deadlock avoided";
        if (ec == ENOMEM)  return "Cannot allocate memory";
        if (ec == ENOTBLK) return "Block device required";
        if (ec == EXDEV)   return "Cross-device link";
        if (ec == ENODEV)  return "Operation not supported by device";
        if (ec == ENFILE)  return "Too many open files in system";
        if (ec == EMFILE)  return "Too many open files";
        if (ec == ENOTTY)  return "Inappropriate ioctl for device";
        if (ec == ETXTBSY) return "Text file busy";
        if (ec == EFBIG)   return "File too large";
        if (ec == ENOSPC)  return "No space left on device";
        if (ec == ESPIPE)  return "Illegal seek";
        if (ec == EMLINK)  return "Too many links";
        if (ec == EPIPE)   return "Broken pipe";
        if (ec == EDOM)    return "Numerical argument out of domain";
        if (ec == ERANGE)  return "Result too large";
        if (ec == EAGAIN)  return "Resource temporarily unavailable";
        if (ec == EWOULDBLOCK) return "Operation would block";
        if (ec == EINPROGRESS) return "Operation now in progress";
        if (ec == EALREADY)    return "Operation already in progress";
        if (ec == ENOTSOCK)    return "Socket operation on non-socket";
        if (ec == EDESTADDRREQ) return "Destination address required";
        if (ec == EMSGSIZE)    return "Message too long";
        if (ec == EPROTOTYPE)  return "Protocol wrong type for socket";
        ...
        if (ec == EOPNOTSUPP || ec == ENOTSUP) return "Operation not supported";
        if (ec == EPFNOSUPPORT)  return "Protocol family not supported";
        if (ec == EAFNOSUPPORT)  return "Address family not supported by protocol family";
        if (ec == EADDRINUSE)    return "Address already in use";
        if (ec == EADDRNOTAVAIL) return "Can't assign requested address";
        if (ec == ENETDOWN)      return "Network is down";
        if (ec == ENETUNREACH)   return "Network is unreachable";
        if (ec == ENETRESET)     return "Network dropped connection on reset";
        if (ec == ECONNABORTED)  return "Software caused connection abort";
        if (ec == ECONNRESET)    return "Connection reset by peer";
        if (ec == ENOBUFS)       return "No buffer space available";
        if (ec == EISCONN)       return "Socket is already connected";
        if (ec == ENOTCONN)      return "Socket is not connected";
        if (ec == ESHUTDOWN)     return "Can't send after socket shutdown";
        if (ec == ETOOMANYREFS)  return "Too many references: can't splice";
        if (ec == ETIMEDOUT)     return "Operation timed out";
        if (ec == ECONNREFUSED)  return "Connection refused";
        if (ec == ELOOP)         return "Too many levels of symbolic links";
        if (ec == ENAMETOOLONG)  return "File name too long";
        if (ec == EHOSTDOWN)     return "Host is down";
        if (ec == EHOSTUNREACH)  return "No route to host";
        if (ec == ENOTEMPTY)     return "Directory not empty";
        if (ec == EPROCLIM)      return "Too many processes";
        if (ec == EUSERS)        return "Too many users";
        if (ec == EDQUOT)        return "Disc quota exceeded";
        if (ec == ESTALE)        return "Stale NFS file handle";
        if (ec == EREMOTE)       return "Too many levels of remote in path";
        if (ec == EBADRPC)       return "RPC struct is bad";
        if (ec == ERPCMISMATCH)  return "RPC version wrong";
        if (ec == EPROGUNAVAIL)  return "RPC prog. not avail";
        if (ec == EPROGMISMATCH) return "Program version wrong";
        if (ec == EPROCUNAVAIL)  return "Bad procedure for program";
        if (ec == ENOLCK)        return "No locks available";
        if (ec == ENOSYS)        return "Function not implemented";
        if (ec == EFTYPE)        return "Inappropriate file type or format";
        if (ec == EAUTH)         return "Authentication error";
        if (ec == ENEEDAUTH)     return "Need authenticator";
        if (ec == EIDRM)         return "Identifier removed";
        if (ec == ENOMSG)        return "No message of desired type";
        if (ec == EOVERFLOW)     return "Value too large to be stored in data type";
        if (ec == ECANCELED)     return "Operation canceled";
        if (ec == EILSEQ)        return "Illegal byte sequence";
        if (ec == ENOATTR)       return "Attribute not found";
        if (ec == EDOOFUS)       return "Programming error";
        if (ec == EBADMSG)       return "Bad message";
        if (ec == EMULTIHOP)     return "Multihop attempted";
        if (ec == ENOLINK)       return "Link has been severed";
        if (ec == EPROTO)        return "Protocol error";
        if (ec == ENOTCAPABLE)   return "Capabilities insufficient";
        if (ec == ECAPMODE)      return "Not permitted in capability mode";
        if (ec == ENOTRECOVERABLE) return "State not recoverable";
        if (ec == EOWNERDEAD)    return "Previous owner died";
        if (ec == EINTEGRITY)    return "Integrity check failed";
        if (ec == ELAST)         return "Must be equal largest errno";

...
contract getopt is putil_stat {
    ...
    function _main(shell_env e_in, mapping (uint16 => Inode) inodes, mapping (uint16 => bytes) data) internal override pure returns (shell_env e) {
        ...
        string out_redirect = p > 0 ? input.strtok(p, " ") : "";
        string in_redirect = q > 0 ? input.strtok(q, " ") : "";
        string[] args;
        string short_options;
        string[] long_options;
        uint16 action;
        if (!sargs.empty())
            (args, short_options, long_options) = _parse_args(sargs);
    ...
    function _parse_args(string sargs) internal pure returns (string[] args, string short_options, string[] long_options) {
        (string[] tokens, ) = sargs.split(" ");

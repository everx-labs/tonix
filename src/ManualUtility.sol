...
contract ManualUtility is Manual {
    ...
    function _init1() internal override accept {
        ...
        _add_page("dirname", "strip last component from file name", "[OPTION] NAME...", "Output NAME with its last non-slash component and trailing slashes removed; if NAME contains no /'s, output '.' (meaning the current directory).",
            "z", 1, M, [
            "end each output line with NUL, not newline"]);
        _add_page("echo", "display a line of text", "[OPTION]... [STRING]...", "Echo the STRING(s) to standard output.",
            "n", 0, M, [
    ...
    function init2() external override accept {
        ...
        _add_page("head", "output the first part of files", "[OPTION]... [FILE]...", "Print the first 10 lines of each FILE to standard output.  With more than one FILE, precede each with a header giving the file name.",
            "nqvz", 1, M, [
        ...
        _add_page("man", "an interface to the system reference manuals", "[COMMAND]", "man is the system's manual pager. Each page argument given to man is normally the name of a program, utility or function.",
            "a", 0, M, [
    ...
    function init3() external override accept {
        _add_page("paste", "merge lines of files", "[OPTION]... [FILE]...", "Write lines consisting of the sequentially corresponding lines from each FILE, separated by TABs, to standard output.",
            "sz", 1, M, [
            "paste one file at a time instead of in parallel",
            "line delimiter is NUL, not newline"]);
        _add_page("readlink", "print resolved symbolic links or canonical file names", "[OPTION]... FILE...", "Print value of a symbolic link or canonical file name. canonicalize by following every symlink in every component of the given name recursively",
            "femnqvz", 1, M, [
            ...
            "end each output line with NUL, not newline"]);
        _add_page("realpath", "print the resolved path", "[OPTION]... FILE...", "Print the resolved absolute file name; all but the last component must exist",
            "emLPqsz", 1, M, [
            ...
            "end each output line with NUL, not newline"]);
        _add_page("tail", "output the last part of files", "[OPTION]... [FILE]...", "Print the last 10 lines of each FILE to standard output.  With more than one FILE, precede each with a header giving the file name.",
            "nqvz", 1, M, [
            "output the last NUM lines, instead of the last 10;  or use -n +NUM to output starting with line NUM",
            "never output headers giving file names",
            "always output headers giving file names",
            "line delimiter is NUL, not newline"]);
        _add_page("wc", "print newline, word, and byte counts for each file", "[OPTION]... [FILE]...", "Print newline, word, and byte counts for each FILE, and a total line if more than one FILE is specified. A word is a non-zero-length sequence of characters delimited by white space.",
            "cmlLw", 1, M, [
        ...
        _add_page("whatis", "display one-line manual page descriptions", "[-dlv] name ...", "Searches the manual page names and displays the manual page descriptions of any name matched.",
            "dlv", 0, M, [

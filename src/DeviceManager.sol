...
contract DeviceManager is Format, SyncFS, ExportFS, CacheFS {
    ...
    uint8 _devc;
    Mount[] _mnt;
    MountInfo[] public _boot_mounts;
    MountInfo[] public _static_mounts;
    MountInfo[] public _current_mounts;
    DeviceInfo[] public _devices;

    function try_mount(MountInfo mnt) external accept {
        _try_mount(mnt);
    }

    function _try_mount(MountInfo mnt) internal {
    ...
    function _try_unmount(MountInfo mnt) internal {
    ...
    function _add_device_files(uint8 major_id, string[] names, uint[] addresses) internal {
    ...
    function _init() internal override accept {
        ...
        _static_mounts = [
            ...
            MountInfo(7, 1, 13, "/sys/dev/block", MOUNT_DIR),
            MountInfo(7, 2, 14, "/sys/dev/char", MOUNT_DIR),
            MountInfo(2, 1, 8, "/usr", MOUNT_DIR),
            MountInfo(3, 1, 1, "/bin", MOUNT_DIR),
            MountInfo(4, 1, 1, "/bin", MOUNT_DIR),
    ...
    function init2() external accept {
        _add_device_files(FT_BLKDEV,
            ["BlockDevice", "DataVolume", "ManualCommands", "ManualStatus", "ManualSession", "ManualUtility", "DeviceManager"], [
        ...
        _sb_exports.push(_get_export_sb(ROOT_DIR + 3, 7, "block"));
        this.init3();
    }
    ...
    function init3() external accept {
        ...
        _sb_exports.push(_get_export_sb(ROOT_DIR + 3 + 7, 4, "char"));
    ...
    /* Query devices and file systems status */
    function dev_admin(SessionS session, InputS input, ArgS[] arg_list) external accept {
        (uint8 c, string[] args, uint flags) = input.unpack();
        uint16 pid = session.pid;
        if (!arg_list.empty())
            pid = pid;
    ...
    function _dev_admin(uint8 c, uint flags, string[] args) private returns (string out) {
        ...
        if (c == mount) _mount(flags, args);// 800
    ...
    function _umount(uint flags, string[] args) private returns (string out) {
        ...
        if (unmount_all) {
            ...
            if (index > 0)
                _try_unmount(MountInfo(_devices[index - 1].major_id, 1, _resolve_absolute_path(s_target), s_target, MOUNT_DIR));
        }
    }
...
//        bool move_subtree = (flags & _M) > 0;
        ...
        if (mount_all) {
            ...
            uint8 index = _device_index(s_source);
            if (index > 0)
                _try_mount(MountInfo(_devices[index - 1].major_id, 1, _resolve_absolute_path(s_target), s_target, MOUNT_DIR));
        }
    }
...
//        bool search_kernel = (flags & _k) > 0;
    ...
    function _lsblk(uint flags, string[] args) private view returns (string out) {
        ...
        if (block_dev_dir_ft != FT_DIR)
            return "Error: could not open /sys/dev/block/\n";
        if (char_dev_dir_ft != FT_DIR)
            return "Error: could not open /sys/dev/char/\n";

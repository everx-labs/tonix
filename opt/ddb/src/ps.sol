...
contract ps {
    ...
    // At least one non-optional show-command must be implemented using DB_SHOW_ALL_COMMAND() so that db_show_all_cmd_set gets created. Here is one.
    DB_SHOW_ALL_COMMAND(procs, db_procs_cmd) internal {
    	db_ps(addr, have_addr, count, modif);
    }

    function dump_args(volatile struct proc *p) internal {
    	char *args;
    	int i, len;
    	if (p.p_args == NULL)
    		return;
    ...
    function db_ps(db_expr_t addr, bool hasaddr, db_expr_t count, char *modif) internal {
    	s_proc p;
    	int i;
    ...
    function db_ps_proc(struct proc *p) internal {
    	s_proc pp;
    	s_thread td;
    	s_ucred cred;
    	s_pgrp pgrp;
    	...
    	cred = p.p_ucred;
    	pgrp = p.p_pgrp;
    	db_printf("%5d %5d %5d %5d ", p.p_pid, pp.p_pid, pgrp != NULL ? pgrp.pg_id : 0, cred != NULL ? cred.cr_ruid : 0);
    	...
    	case PRS_NORMAL:
    		if (P_SHOULDSTOP(p))
    			...
    			// One of D, L, R, S, W.  For a multithreaded process we will use the state of the thread with the highest precedence.  The
    			// precendence order from high to low is R, L, D, S, W.  If no thread is in a sane state we use '?' for our primary state.
    			rflag = sflag = dflag = lflag = wflag = 0;
    			FOREACH_THREAD_IN_PROC(p, td) {
    				if (TD_GET_STATE(td) == TDS_RUNNING || TD_GET_STATE(td) == TDS_RUNQ || TD_GET_STATE(td) == TDS_CAN_RUN)
    					rflag++;
    				if (TD_ON_LOCK(td))
    					lflag++;
    ...
    function dumpthread(volatile struct proc *p, volatile struct thread *td, int all) internal {
    ...
    function DB_SHOW_COMMAND(thread, db_show_thread) internal {
    	s_thread td;
    	u_int delta;
    	bool comma;
    	...
    	if (td.td_name[0] != '\0')
    		db_printf(" name: %s\n", td.td_name);
    	db_printf(" pcb: %p\n", td.td_pcb);
    	db_printf(" stack: %p-%p\n", td.td_kstack, (td.td_kstack + td.td_kstack_pages * PAGE_SIZE - 1));
    ...
    function DB_SHOW_COMMAND(proc, db_show_proc) internal {
    	struct thread *td;
    	struct proc *p;
    	int i;
    ...
    function db_findstack_cmd(db_expr_t addr, bool have_addr, db_expr_t dummy3 __unused, char *dummy4 __unused) internal {
    	s_thread td;
    	vm_offset_t saddr;
    	if (have_addr)
    		saddr = addr;

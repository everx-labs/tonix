...
contract du is putil_stat {
    ...
    function _main(shell_env e_in, mapping (uint16 => Inode) inodes, mapping (uint16 => bytes) data) internal override pure returns (shell_env e) {
        ...
        for (string param: params) {
            ...
            if (!f.ferror()) {
                ...
                (, uint16 st_ino, uint16 st_mode, , , , , uint32 st_size, , , , ) = libstat.st_attrs(f.attr);
                uint16 index = st_ino;
                Inode ino = inodes[index];
                bytes dir_data = data[index];
                uint32 file_size = st_size;
                (string[][] table, uint32 total) = libstat.is_dir(st_mode) ? _count_dir(count_files, include_subdirs, human_readable, param, ino, dir_data, inodes, data) :
    ...
    function _count_dir(bool count_files, bool include_subdirs, bool human_readable, string dir_name, Inode ino, bytes dir_data, mapping (uint16 => Inode) inodes, mapping (uint16 => bytes) data) private pure returns (string[][] lines, uint32 total) {

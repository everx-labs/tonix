...
contract readlink is putil_stat {
    ...
    function _main(shell_env e_in, mapping (uint16 => Inode) inodes, mapping (uint16 => bytes) data) internal override pure returns (shell_env e) {
        ...
        for (string param: params) {
            ...
            if (canon)
                ...
                Arg arg = _dereference(mode + libpath.EXPAND_SYMLINKS, param, wd, inodes, data);
                (spath, t, , , ) = arg.unpack();
    ...
    function _canonicalize(uint16 mode, string param, uint16 wd, mapping (uint16 => Inode) inodes, mapping (uint16 => bytes) data) internal pure returns (string res, bool valid) {
        ...
        (string arg_dir, string arg_base) = libpath.dir(param);
        bool is_abs_path = param.substr(0, 1) == "/";
        valid = true;

        if (canon_mode >= libpath.CANON_DIRS) {
        ...
        res = canon_mode == libpath.CANON_NONE || (canon_mode == libpath.CANON_MISS || canon_mode == libpath.CANON_EXISTS) && is_abs_path ?
            param : canon_mode == libpath.CANON_DIRS ? fs.xpath(arg_dir, fs.resolve_absolute_path(arg_dir, inodes, data), inodes, data) + "/" + arg_base : fs.xpath(param, wd, inodes, data);
    }
    ...
    function _dereference(uint16 mode, string param, uint16 wd, mapping (uint16 => Inode) inodes, mapping (uint16 => bytes) data) internal pure returns (Arg) {
        bool expand_symlinks = (mode & libpath.EXPAND_SYMLINKS) > 0;
        ...
        if (expand_symlinks && t == libstat.FT_SYMLINK) {
            (t, param, ino) = udirent.get_symlink_target(inod, data[ino]).unpack();
        }

    ...
    function main(s_proc p_in, mapping (uint16 => Inode) inodes, mapping (uint16 => bytes) data) external pure returns (s_proc p) {
                ...
                if (t == libstat.FT_DIR) {
                    if (remove_empty_dirs) {
                        if (inodes[iop].n_links < 3) {
                            ...
                            errors.push(Err(0, liberr.ENOTEMPTY, s));
                    } else
                        errors.push(Err(0, liberr.EISDIR, s));
                } else {
                    ars.push(Ar(aio.UNLINK, iop, s, ""));
        ...
        for ((uint16 dir_i, string[] victim_dirents): victims)
            if (!victim_dirents.empty())
                ars.push(Ar(aio.UPDATE_DIR_ENTRY, dir_i, "", _remove_dir_entries(string(data[dir_i]), victim_dirents)));
    }

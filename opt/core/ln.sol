...
//        s_dirent[] contents = p.p_args.ar_misc.pos_args;
        ...
        if (dest_exists && t_ft != libstat.FT_DIR) {
            if (multiple_sources)
                errors.push(Err(er.ln_target, liberr.ENOTDIR, t_path));
            else if (!force)
                errors.push(Err(symlink ? er.failed_symlink : er.failed_hardlink, liberr.EEXIST, t_path));
        }
...
//            return (out, ars, errors);
        ...
        if (request_backup && overwrite_dest) {
            ...
            out.aif(verbose, "(backup:" + str.squote(t_backup_path) + ")");
        ...
        for (uint i = first; i < last; i++) {
            ...
            if (sft == libstat.FT_DIR && aop == aio.HARDLINK)
                ...
                (, string file_name) = libpath.dir(to_dir ? spath : t_path);

...
contract hash is pbuiltin_base {
    ...
    function main(shell_env e_in, job_cmd cc_in) external pure returns (shell_env e, job_cmd cc) {
        ...
        (bool print_tabbed, bool print_reusable, bool forget_some, bool forget_all, bool use_pathname, , , ) = cc.flag_values("tldrp");
...
//                    string item = vars.get_pool_record(param, page_in);
                    if (!path.empty())
                        e.puts(_display_item(path, param, print_reusable, print_tabbed));
                }
            }

...
/*    function a2_stack(byte b, byte n) internal pure returns (string out, uint sh) {
        sh = 1;
    ...
    function a3_tuple(byte b, byte n) internal pure returns (string out, uint sh) {
        sh = 1;
    ...
    function a4_constant(bytes bb) internal pure returns (string out, uint sh, uint cnt) {
...
//    function swap_uint32(uint32 val) internal pure returns (uint32) {
//        val = ((val << 8) & 0xFF00FF00) | ((val >> 8) & 0xFF00FF);
//        return (val << 16) | (val >> 16);
//    }
//
//    function swap_int32(int32 val) internal pure returns (int32) {
//        val = ((val << 8) & int32(0xFF00FF00)) | ((val >> 8) & int32(0xFF00FF));
//        return (val << 16) | ((val >> 16) & 0xFFFF);
//    }
    function a5_arithmetic(byte b, byte n, byte na) internal pure returns (string out, uint sh) {
        sh = 1;
    ...
    function other_comparison(byte n) internal pure returns (string out) { // C7
        // 00 - 13
    ...
    function int_comparison(byte b, byte n) internal pure returns (string out, uint sh) { // C7
        sh = 1;
        if (b == 0xB8) out = "SGN";
    ...
    function a6_comparison(byte b, byte n) internal pure returns (string out, uint sh) {
        sh = 1;
    ...
    function a7_cell(byte b, byte n, byte na) internal pure returns (string out, uint sh) {
        sh = 1;
        // A.7 Cell primitives
...
//    function a8_continuation(byte b, byte n, byte na) internal pure returns (string out, uint sh, uint cnt) {
    function a8_continuation(byte b, byte n, byte na) internal pure returns (string out, uint sh) {
        sh = 1;
...
//            }
    ...
    function a9_exception(byte b, byte n) internal pure returns (string out, uint sh) {
        sh = 1;
    ...
    function a10_dictionary(byte b, byte n) internal pure returns (string out, uint sh) {
        sh = 1;
        // A.10 Dictionary manipulation primitives
        // F4 - F7?
        if (b == 0xF4) { // A.10.2. Dictionary serialization and deserialization.
            sh++;
            // 00 - BF
    ...
    function gas_related(byte n) internal pure returns (string out) { // F8
        // 00 - 0F
    ...
    function pseudo_random(byte n) internal pure returns (string out) { // F8
        // 10 - 1F
    ...
    function config(byte n) internal pure returns (string out) { // F8
        // 20 - 3F
        if (n == 0x23) out = "NOW";
    ...
    function global(byte n) internal pure returns (string out) { // F8
        // 40 - FF
    ...
    function crypto(byte n) internal pure returns (string out) { // F9
        // 10 - 1F
    ...
    function a11_application(byte b, byte n) internal pure returns (string out, uint sh) {
        sh = 1;
        // A.11 Application-specific primitives
        // F8 - FB
        if (b == 0xF8) {
            sh++;
            if (n < 0x10) out = gas_related(n); // A.11.2. Gas-related primitives.
            else if (n < 0x20) out = pseudo_random(n); // A.11.3. Pseudo-random number generator primitives.
            else if (n < 0x40) out = config(n); // A.11.4. Configuration primitives.
            else out = global(n); // A.11.5. Global variable primitives
        } else if (b == 0xF9) { // A.11.6. Hashing and cryptography primitives.
            sh++;
            out = crypto(n);
//        } else out = format("{:X}", uint8(b));
    ...
    function a12_debug(byte n) internal pure returns (string out, uint sh) {
        sh = 2;
    ...
    function a13_codepage(byte n) internal pure returns (string out, uint sh) {
        sh = 2;
    ...
    function is_ref(byte b, byte n) internal pure returns (bool res) {
...
/*    function _dda(bytes bb) internal pure returns (string out) {
        uint len = bb.length;
        if (len == 0)
            return "";
        ...
        while (pos < len - 2) {
            uint cnt;
            (byte b, byte n, byte na) = (bb[pos], bb[pos + 1], bb[pos + 2]);
...
//            else if (b < 0xF2 || b == 0xF2 && n < 0x20) (sout, sh, cnt) = a8_continuation(b, n, na);
            else if (b < 0xF2) (sout, sh) = a8_continuation(b, n, na);
//            else if (b < 0xF2) (sout, sh, cnt) = a8_continuation(b, n, na);
            else if (b < 0xF4) (sout, sh) = a9_exception(b, n);
    ...
    function ppa(string insn, byte arg) internal pure returns (string out) {
        return format("{}{}", insn, uint8(arg));
    }
    function ppia(string insn, byte arg) internal pure returns (string out) {
        TvmBuilder d;
        d.store(arg);
        TvmSlice s1 = d.toSlice();
        int8 ia = s1.loadSigned(d.bits());
        return format("{} {}", insn, ia);
    }
    function ppa1(string insn, byte arg) internal pure returns (string out) {
        return format("{} {}", insn, uint16(uint8(arg)) + 1);
    }
    function ppa2(string insn, byte arg, int16 offset) internal pure returns (string out) {
        return format("{}{}", insn, uint8(arg) - offset);
    }
    function uk(string ctg, byte arg) internal pure returns (string out) {
